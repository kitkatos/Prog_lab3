...
public abstract class Factory {
    ...
    public void addNewWorker(Worker worker, int salary) {
        int slot = ArrayUtils.getFirstEmptySlot(this.workersInformations);
    ...
    public void getFired(Worker worker) {
        int index = getWorkerIndex(worker);

        if(index == -1) {
            throw new IllegalArgumentException("На фабрике нет такого работника");
        }

        this.workersInformations[index] = null;
        System.out.printf("Работник %s уволился с фабрики %s\n", worker.getName(), this.name);
    }

    private int getWorkerIndex(Worker worker) {
        for(int i = 0; i < this.workersInformations.length; i++) {
            if(this.workersInformations[i] == null) {
                continue;
            }

            if(this.workersInformations[i].getWorker().equals(worker)) {
                return i;
            }
        }
